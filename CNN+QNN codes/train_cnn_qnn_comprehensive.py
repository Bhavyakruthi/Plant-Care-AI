...
print("=" * 100)
print("  MODEL 2: CNN + QNN (Classical-Quantum Hybrid)")
...
os.makedirs('results/cnn_qnn', exist_ok=True)
...
quantum_in_dim = n_qubits

print(f"Quantum Configuration:")
print(f"  Qubits: {n_qubits}")
print(f"  Layers: {n_layers}")
print(f"  Input dimension: {quantum_in_dim}")
print()
...
class QuantumClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        # Classical expansion
        out = self.expansion(quantum_out.float())
        return out


# Initialize model
print("Initializing Quantum classifier...")
...
quantum_params = n_qubits * n_layers * 2
...
checkpoint = torch.load('models/cnn_qnn_best.pt')
...
plt.title('CNN + QNN - Confusion Matrix', fontsize=16, fontweight='bold', pad=20)
plt.ylabel('True Label', fontsize=12, fontweight='bold')
plt.xlabel('Predicted Label', fontsize=12, fontweight='bold')
plt.tight_layout()
plt.savefig('results/cnn_qnn/confusion_matrix.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('CNN + QNN - Normalized Confusion Matrix', fontsize=16, fontweight='bold', pad=20)
plt.ylabel('True Label', fontsize=12, fontweight='bold')
plt.xlabel('Predicted Label', fontsize=12, fontweight='bold')
plt.tight_layout()
plt.savefig('results/cnn_qnn/confusion_matrix_normalized.png', dpi=300, bbox_inches='tight')
plt.close()
...
# Loss curves
ax = axes[0, 0]
ax.plot(train_losses, label='Train Loss', linewidth=2, color='#2ecc71')
...
# Accuracy curves
ax = axes[0, 1]
ax.plot(train_accs, label='Train Acc', linewidth=2, color='#2ecc71')
...
plt.tight_layout()
plt.savefig('results/cnn_qnn/training_curves.png', dpi=300, bbox_inches='tight')
plt.close()
...
for i in range(n_classes):
    ...
    axes[i].plot(fpr, tpr, color='green', lw=2, label=f'ROC (AUC = {roc_auc:.3f})')
...
plt.suptitle('CNN + QNN - ROC Curves (One-vs-Rest)', fontsize=16, fontweight='bold', y=0.995)
plt.tight_layout()
plt.savefig('results/cnn_qnn/roc_curves.png', dpi=300, bbox_inches='tight')
plt.close()
...
# Precision
axes[0].bar(x, precision_per_class, width, color='#2ecc71', edgecolor='black', linewidth=1.2)
...
# Recall
axes[1].bar(x, recall_per_class, width, color='#3498db', edgecolor='black', linewidth=1.2)
...
plt.suptitle('CNN + QNN - Per-Class Performance Metrics', fontsize=16, fontweight='bold', y=0.98)
plt.tight_layout()
plt.savefig('results/cnn_qnn/per_class_metrics.png', dpi=300, bbox_inches='tight')
plt.close()
...
colors_metric = ['#2ecc71', '#3498db', '#f39c12', '#e74c3c', '#9b59b6']
...
model_info = [
    ...
    f"Quantum Params: {quantum_params:,}",
    f"Model Size: {model_size:.2f} MB",
    f"Qubits: {n_qubits}",
    f"Quantum Layers: {n_layers}",
...
sns.heatmap(cm_normalized, annot=False, cmap='Greens', cbar=True, ax=ax5, square=True)
...
ax6.bar(range(n_classes), class_counts, color='#2ecc71', edgecolor='black', linewidth=1)
...
plt.suptitle('CNN + QNN - Comprehensive Performance Summary', fontsize=16, fontweight='bold')
plt.savefig('results/cnn_qnn/performance_summary.png', dpi=300, bbox_inches='tight')
plt.close()
...
# Save JSON
with open('results/cnn_qnn/results_detailed.json', 'w') as f:
    json.dump(results, f, indent=2)
print("✓ Detailed results saved to JSON")

# Save text report
with open('results/cnn_qnn/results_report.txt', 'w') as f:
    f.write("=" * 100 + "\n")
    f.write("CNN + QNN - COMPREHENSIVE EVALUATION REPORT\n")
    f.write("=" * 100 + "\n\n")
    
    f.write("MODEL INFORMATION\n")
    f.write("-" * 100 + "\n")
    f.write(f"Model Type: Classical-Quantum Hybrid\n")
    f.write(f"Feature Extractor: ResNet18 CNN (pretrained)\n")
    f.write(f"Classifier: Variational Quantum Classifier\n")
    f.write(f"Quantum Circuit: {n_qubits} qubits, {n_layers} layers\n")
    f.write(f"Total Parameters: {total_params:,}\n")
    f.write(f"Quantum Parameters: {quantum_params:,}\n")
    f.write(f"Classical Parameters: {total_params - quantum_params:,}\n")
    ...
    f.write("QUANTUM INFORMATION\n")
    f.write("-" * 100 + "\n")
    f.write(f"Number of Qubits: {n_qubits}\n")
    f.write(f"Circuit Depth (layers): {n_layers}\n")
    f.write(f"Quantum Parameters: {quantum_params:,}\n")
    f.write(f"Quantum Device: default.qubit\n\n")
...
print("  📁 results/cnn_qnn/results_detailed.json")
print("  📁 results/cnn_qnn/results_report.txt")
print("  📊 results/cnn_qnn/*.png (6 visualization files)")
print("  💾 models/cnn_qnn_best.pt")
print()
